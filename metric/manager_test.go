...
func TestManagerShouldHandleNewMetric(t *testing.T) {
	...
	ms0.On("GetPattern").Return(metric.Pattern(".*")).Once()
	ms0.On("AddMetric", metricName).Return().Once()
	ms0.On("Start").Return().Once()

	ms1 := new(mock.Supervisor)
	ms1.On("GetPattern").Return(metric.Pattern("\\d+")).Once()

	ms2 := new(mock.Supervisor)
	ms2.On("GetPattern").Return(metric.Pattern(".*")).Once()
